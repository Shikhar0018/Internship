...
class _SpacesCardState extends State<SpacesCard> {
  ...
  Future<SpacesModel> _getData(HomeConstants constants) async {
    SharedPreferences _prefs = await SharedPreferences.getInstance();
    String? _authToken = _prefs.getString(PrefsConstants.authToken) == null
        ? ""
        : _prefs.getString(PrefsConstants.authToken);
    String? _userID = _prefs.getString(PrefsConstants.userID) == null
        ? ""
        : _prefs.getString(PrefsConstants.userID);
    ...
    switch (constants) {
      case HomeConstants.SPACES:
        ...
        response = await http.post(
            Uri.parse(APIConstants.grow90_base_url +
                '/api/rest/v2/get_unjoined_spaces'),
            headers: {
                                ...
                                Container(
                                  ...
                                  child: NeuLeadButton(
                                    onPressed: () async {
                                      SharedPreferences _prefs =
                                          await SharedPreferences.getInstance();
                                      String _authToken = _prefs.getString(
                                              PrefsConstants.authToken) ??
                                          '';
                                      String _userID = _prefs.getString(
                                              PrefsConstants.userID) ??
                                          '';
                                      http.Response response;
                                      response = await http.post(
                                          Uri.parse(APIConstants.db_base_url +
                                              '/api/rest/v2/join_spaces'),
                                          headers: {
                                            "Authorization":
                                                "Bearer $_authToken"
                                          },
                                          body: jsonEncode({
                                            "track_id": _thisTrack.id,
                                            "user_id": _userID
                                          }));

                                      var data = json.decode(response.body);
                                      // log(data.toString());
                                      _prefs.setString(AppConstants.spaceID,
                                          _thisTrack.id ?? '');
                                      _prefs.setString(AppConstants.spaceName,
                                          _thisTrack.title ?? '');
                                      Phoenix.rebirth(context);
